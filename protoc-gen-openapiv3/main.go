...
var (
	...
	defaultErrorResponseSchema     = flag.String("default_error_response_schema", "", "fully-qualified message name to use as the body schema for description-only error responses (4xx/5xx). When empty, falls back to google.rpc.Status. The message — and any message/enum types it references — must be part of the generated document so their component schemas are emitted (automatic with allow_merge, where all input protos are merged).")
...
func main() {
	...
	reg.SetDefaultErrorResponseSchema(*defaultErrorResponseSchema)
